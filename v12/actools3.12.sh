...
setup_stack() {
  ...
  info "Installing trust command for internal TLS if missing..."
  command -v trust >/dev/null || run "apt-get install -y libnss3-tools"
...
EOF
  ...
  docker compose run --rm caddy caddy validate --config /etc/caddy/Caddyfile || error "Caddyfile validation failed"
