...
setup_stack() {
  ...
  # Create internal CA for dev/stg
  INTERNAL_CA="$REAL_HOME/.actools-internal-ca"
  mkdir -p "$INTERNAL_CA"
  if [[ ! -f "$INTERNAL_CA/root.crt" ]]; then
      info "Generating internal CA for dev/stg HTTPS on host"
      caddy trust --cert-file "$INTERNAL_CA/root.crt"
  fi
...
main() {
  ...
  echo
  echo "Tips:"
  echo "- Use 'curl -k https://dev.$BASE_DOMAIN' for dev/stg self-signed TLS"
