...
preflight_checks() {
  ...
  if ! groups "$REAL_USER" | grep -q "\bdocker\b"; then
    warn "User $REAL_USER is not in docker group. Adding..."
    usermod -aG docker "$REAL_USER"
    info "User $REAL_USER added to docker group. You must log out and back in."
  fi

  # Ports 80/443
  for port in 80 443; do
    if ss -ltnp | grep -q ":$port"; then
      error "Port $port is already in use. Stop other services before running."
    fi
  done

  # Writable home and docroot
  [[ -w "$REAL_HOME" ]] || error "Home directory $REAL_HOME is not writable"
  mkdir -p "$REAL_HOME/docroot"
  [[ -w "$REAL_HOME/docroot" ]] || error "Docroot $REAL_HOME/docroot is not writable"

  # Trust command
  if ! command -v trust &>/dev/null; then
    info "Installing trust command..."
...
main() {
  ...
  echo
  echo "Extra tip: For production HTTPS with Let's Encrypt, Caddy automatically requests certs. For dev/stg, self-signed certs are used."
}
