...
# =============================================================================
# DOCKER STACK WITH CADDY HTTPS SELF-SIGNED
# =============================================================================
setup_stack() {
...
services:
  ...
  db:
    ...
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-uroot", "-p$DB_ROOT_PASS"]
      interval: 2s
      retries: 15
...
# =============================================================================
# DB READINESS (healthcheck-based)
# =============================================================================
wait_db() {
...
Extra tip:
If you want real Drupal + HTTPS routing later, you can replace the self-signed Caddy TLS by:
1. Replacing the auto-generated Caddyfile with one using:
   tls $REAL_USER@example.com
2. Ensure ports 80/443 are publicly reachable.
3. Caddy will automatically obtain real Let's Encrypt certs.
