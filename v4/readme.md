...
📘 2. Enterprise README (Single Page)
# Actools Enterprise Installer v4

## Overview

Actools is a **single-command platform installer** for:

- Drupal 11 multi-environment setup (dev/stg/prod)
- Docker-based infrastructure
- Secure, state-driven deployments

This is **not just an installer** — it is a **server lifecycle tool**.
...
3. **State-Driven Deployments**
4. **Safe by Default (backup + confirmation)**
5. **App is disposable, infrastructure is controlled**
...
- Per-environment DB isolation
- Automatic password generation
- State tracking (`~/.actools-state.json`)
- Backup before destructive operations
- Lock file protection
- Wait-for-DB reliability
- DRY RUN support
...
sudo ./actools-installer.sh rerun --force
Dry run
DRY_RUN=true sudo ./actools-installer.sh fresh
State File
...
Harder to maintain vs modular structure

❌ Tight Coupling

Docker + Drupal assumptions baked in

❌ Limited Multi-Server Support

Currently single-node only

❌ Secrets in Local State

Not suitable for high-compliance environments yet

When to Use

✔ Single server deployments
✔ Small to medium production stacks
...
❌ High compliance (PCI, FedRAMP) without extensions

Future Roadmap

CLI tool (actools install, actools backup)

Modular architecture (lib/, commands/)

Remote server provisioning

Security layering (armourinstaller)

Secrets manager integration

Final Philosophy

Actools does not just install Drupal.
It defines how your server behaves.


---

# 🧠 Final Thought (Architect to Architect)

What you now have is:

👉 A **controlled system bootstrapper**  
👉 Not just an installer  

If you keep evolving this cleanly:

- Add CLI → becomes tool  
- Add remote → becomes platform  
- Add policy → becomes infrastructure layer  

---

If you want next:

👉 I can convert this into a **real `actools` CLI (binary-style UX like ddev)**  
👉 Or split into **modular architecture without losing simplicity**
