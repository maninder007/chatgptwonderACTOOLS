...
# =============================================================================
# Actools Enterprise Installer v3.11c (Ubuntu 24.04)
# =============================================================================

ACTOOLS_VERSION="3.11c"
MODE="${1:-fresh}"
FORCE=false
[[ "${2:-}" == "--force" ]] && FORCE=true
...
# =============================================================================
# DOCKER STACK + CADDY HTTPS AUTO
# =============================================================================
setup_stack() {
  # Auto-generate self-signed Caddyfile if missing
  CADDYFILE="$REAL_HOME/Caddyfile"
  if [[ -d "$CADDYFILE" ]]; then rm -rf "$CADDYFILE"; fi
  [[ -f "$CADDYFILE" ]] || cat <<EOF > "$CADDYFILE"
# Auto-generated Caddyfile (self-signed)
dev.$BASE_DOMAIN {
...
EOF
  ...
  # docker-compose.yml with HEALTHCHECK for DB
...
wait_db() {
  ...
  for i in {1..30}; do
    ...
    [[ "$status" == "healthy" ]] && { info "DB is healthy"; return; }
    sleep 2
  done
...
Extra tip:
- To replace self-signed certs with real Let's Encrypt certs later, modify the Caddyfile:
